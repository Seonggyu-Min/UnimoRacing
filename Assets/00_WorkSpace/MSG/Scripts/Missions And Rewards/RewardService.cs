...
namespace MSG
{
    public class RewardService : MonoBehaviour
    {
        ...
        private void Start()
        {
            InGameManager.Instance.OnRaceState_Finish += Give; // 모든 플레이어가 들어오면 호출. 그리고 PostGame은 코루틴 종료 후 Invoke 돼서 늦을 수도?
        }
        ...
        private void Give()
        {
            ...
            {
                string finished = PhotonNetworkCustomProperties.GetPlayerProp(
                    p.Value, 
                    PlayerKey.RaceFinishedTime, 
                    string.Empty
                    );

                if (!float.TryParse(finished, out float finishedTime))
                {
                    Debug.LogWarning("[RewardService] finishedTime 파싱 실패");
                }
